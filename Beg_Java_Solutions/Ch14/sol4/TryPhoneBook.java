...
/*
   To allow searching using just a second name you need a new strategy for
   how you store entries in the map. You can't just use second names as keys
   for BookEntry values because it is more than likely there will be duplicate
    second names from time to time, and duplicate keys are not allowed.
   
   One approach is to use the second name as a key and store a linked list of 
   ...
   To continue to allow searching for a number, you will also keep the second map with 
   PhoneNumber objects as keys, but extend it to allow multiple entries per number by
   making the value for a key a linked list of entries.
