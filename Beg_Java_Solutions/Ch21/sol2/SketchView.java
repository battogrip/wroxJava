...
class SketchView extends JComponent implements Observer, ActionListener, Printable, Pageable {
  ...
  public int print(Graphics g,              // Graphics context for printing
    ...
    // Draw box around where the sketch will go
    g2D.setPaint(Color.black);
    int pageWidth = (int)pageFormat.getImageableWidth();
    int pageHeight = (int)pageFormat.getImageableHeight();
    g2D.drawLine(0, 0, pageWidth, 0);
    g2D.drawLine(pageWidth, 0, pageWidth, pageHeight);
    g2D.drawLine(pageWidth, pageHeight, 0, pageHeight);
    g2D.drawLine(0, pageHeight, 0, 0);

    // Code to print sketch name
    ...
    Rectangle2D.Float textRect = (Rectangle2D.Float)font.getStringBounds(
                                      sketchName, g2D.getFontRenderContext());
    double centerX = pageFormat.getImageableWidth()/2;

    // draw text in the middle of the printable area
    g2D.setPaint(Color.blue);
    g2D.drawString(sketchName, (float)(centerX-textRect.width/2),
                               (float)textRect.height);
    // End of code to print sketch name
