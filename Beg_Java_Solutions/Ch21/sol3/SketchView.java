...
class SketchView extends JComponent implements Observer, ActionListener, Printable, Pageable {
  ...
  // Method to return page count - now 5 pages
  ...
  public int print(Graphics g,              // Graphics context for printing
    ...
    // Calculate the scale to fit sketch to four pages
    double scaleX = 2.0 * pageFormat.getImageableWidth()/rect.width;
    double scaleY = 2.0 * pageFormat.getImageableHeight()/rect.height;

    // Get minimum scale factor
    double scale = Math.min(scaleX, scaleY);  
    System.out.println("scaleX = " + scaleX+" scaleY = " + scaleY + " ratio = "+scaleX/scaleY);
    ...
    // Move the origin depending on which page is being printed
    // We must take account of scaling to pplace the origin for the right
    // or bottom pages correctly
    // Because we chose the minimum scale and applied that to both axes, the axis that
    // required the minimum scale need to be translated further to put it in the correct position
    if ((pageIndex == 2) || (pageIndex == 4)) {                // For the two pages on the right
      g2D.translate((-rect.width/2.0)*(scaleX/scale), 0.0);    // Shift the origin in x 
    } 
    if ((pageIndex == 3) || (pageIndex == 4)) {                // For the two pages at the bottom
      g2D.translate(0.0, (-rect.height/2.0)*(scaleY/scale));   // Shift the origin in y
    }
