...
/*
  The valueChanged() method has been changed to simply store the values of tableParam and
  columnsParam, rather than actually performing the database query. 
  The SQL query string is assembled, and the query performed, in the actionPerformed() method
  when the "Perform Query" button is clicked. The updateStatus() method displays appropriate text
  in the status bar; the boolean parameter to this method is true if the query has actually been
  performed, and false if tables/columns have been selected but the query has not been executed.
      ...
      if ((columnsToView != null) && (tableToView != null)) {
        try {
          ...
          tableModel.setResultSet(
               statement.executeQuery("SELECT " + columnsToView+" FROM " + tableToView));
