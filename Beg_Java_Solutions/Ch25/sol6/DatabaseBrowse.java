...
/*
  This displays a further JOptionPane to allow an ORDER BY clause to be entered.
...
class DatabaseBrowse extends JFrame implements ActionListener, TreeSelectionListener {
  ...
  public DatabaseBrowse() {
    ...
    // Box container for query execute button
    Box queryPane = Box.createHorizontalBox();
    performQueryButton.addActionListener(this);
    queryPane.add(performQueryButton);

    Box inputPane = Box.createVerticalBox();
    inputPane.add(dbPane);
    inputPane.add(loginPane);
    inputPane.add(queryPane);                            // Box holding query input components
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    } else if (source == performQueryButton) {
      if ((columnsToView != null) && (tableToView != null)) {
        StringBuffer queryString = new StringBuffer("SELECT ");
        queryString.append(columnsToView).append(" FROM ").append(tableToView);

       String where = JOptionPane.showInputDialog(this,
          "If you wish to select only certain rows from the table,\n" +
          "enter the body of a WHERE clause below and click OK.\n\nOtherwise, click Cancel.",
          "Table row selection", JOptionPane.QUESTION_MESSAGE);
        if ((where != null) || (!where.equals(""))) {
          queryString.append(" WHERE ").append(where);
        }

        String order = JOptionPane.showInputDialog(this,
          "If you wish to order the query results in a particular way,\n" +
          "enter the body of an  ORDER BY clause and click OK.\n\nOtherwise, click Cancel.",
          "Result ordering", JOptionPane.QUESTION_MESSAGE);
        if ((order != null) || (!order.equals(""))) {
          queryString.append(" ORDER BY ").append(order);
        }

        try {
          // Display the columns and change the scroll pane border
          tableModel.setResultSet(
               statement.executeQuery(queryString.toString()));
          ...
          String message = "SQL Error performing SELECT\n" + sqle.getMessage();
  ...
  public void valueChanged(TreeSelectionEvent e) {
    ...
    tableToView = tableParam;
    columnsToView = columnsParam;

    updateStatus(false);
  }

  void updateStatus(boolean queryPerformed) {
    String message = null;
    if (tableToView != null) {
      if (columnsToView.equals("*")) {
        message = "complete table " + tableToView;
      } else {
        message = columnsToView + " from table " + tableToView;
      }
  ...
  private JButton performQueryButton = new JButton("Perform query");
