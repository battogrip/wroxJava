...
/*
  Here we add the ability to query the file by entering a name from the keyboard.
  First the index  is searched to find positions in the persons file corresponding
  to all occurrences of the name. The array of positions are sorted in ascending
...
//     for(int i = 0 ; i<entries.length ; i++) {
      if(name.equals(entry.getName())) {
        positions[foundCount++] = entry.getPosition();
      }
    }
    if(foundCount == 0) {
