...
public class SketchFrame extends JFrame implements ActionListener {
  ...
  public SketchFrame(String title, Sketcher theApp) {
    ...
    colorMenu.add(new JMenuItem(customColorAction = new CustomColorAction()));
   ...
   // Add the font choice item to the options menu
    ...
    popup.add(new JMenuItem(customColorAction));  // Replaces the two statements commented out
...
//    customColorItem.addActionListener(this);     // and add its listener
    ...
    addToolBarButton(customColorAction);         // Replaces commented out statement below
//    addToolBarButton(new ColorDialogAction());
    toolBar.addSeparator();                      // Space at the end

    // This is for the highlighting toggling button
    JButton hiliteButton = addToolBarButton(new HighlightAction());
    hiliteButton.setSelectedIcon(new ImageIcon("Images/hiliteoff.gif"));
    hiliteButton.setSelected(true);
...
//      }
  ...
  // HIGHLIGHT ACTION INNER CLASS
  class HighlightAction extends AbstractAction {
    public HighlightAction()  {
      super("Highlight");
      String iconFileName = "Images/hiliteon.gif";
      if(new File(iconFileName).exists()) {
        putValue(SMALL_ICON, new ImageIcon(iconFileName));
      }
      putValue(SHORT_DESCRIPTION, "Click to toggle highlighting...");
    }

    public void actionPerformed(ActionEvent e) {
        boolean hilite = theApp.getView().toggleHighlighting();
        ((JButton)(e.getSource())).setSelected(hilite);
    }
  }

  // Defines an Action object to select a custom color
  class CustomColorAction extends AbstractAction {
    // Constructor
    public CustomColorAction() {
      super("Custom Color");
      String iconFileName = "Images/ColorDialog.gif";
      if(new File(iconFileName).exists()) {
        putValue(SMALL_ICON, new ImageIcon(iconFileName));
      }
      putValue(SHORT_DESCRIPTION, "Select a custom color...");
    }
  ...
  private CustomColorAction customColorAction;
...
// The following field is no longer required
